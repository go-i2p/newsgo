...
if err := os.WriteFile(sf, []byte("null"), 0644); err != nil {
t.Fatal(err)
}
...
dir := t.TempDir()
sf := filepath.Join(dir, "stats.json")
if err := os.WriteFile(sf, []byte(`{"en_US":5,"de":2}`), 0644); err != nil {
t.Fatal(err)
}
n := &NewsStats{StateFile: sf}
n.Load()
...
dir := t.TempDir()
sf := filepath.Join(dir, "stats.json")
if err := os.WriteFile(sf, []byte(`{not valid json`), 0644); err != nil {
t.Fatal(err)
}
n := &NewsStats{StateFile: sf}
n.Load()
